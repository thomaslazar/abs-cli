...
abs-cli/
  src/
    AbsCli/
      ...
      Commands/
        ...
        MetadataCommand.cs
        TasksCommand.cs
        ChangelogCommand.cs        # Prints bundled CHANGELOG.md (offline)
        ConfigCommand.cs
        LoginCommand.cs
        SelfTestCommand.cs         # AOT integrity verification
        CommandHelper.cs           # Shared client/config resolution
        HelpExtensions.cs          # Custom help sections (Examples, Filter groups, response shapes)
        ResponseExamples.g.cs      # Generated by tools/GenerateResponseExamples
      Services/
        ItemsService.cs
        LibrariesService.cs
        SeriesService.cs
        ...
        MetadataService.cs
        TasksService.cs
        CoversService.cs           # Wraps the four items-cover endpoints
        ChangelogReader.cs         # Parses the embedded CHANGELOG.md resource
      Api/
        AbsApiClient.cs
        ApiEndpoints.cs
        FilterEncoder.cs           # Base64 filter encoding for ABS API
        FilenameSanitizer.cs       # Upload filename normalization (sanitize-drift coverage)
        UnicodeNfc.cs              # Managed NFC (string.Normalize is a no-op under InvariantGlobalization)
        UnicodeNfc.g.cs            # Composition tables, generated by tools/GenerateNfcTables
        TokenHelper.cs             # JWT expiry decoding
      Models/
        JsonContext.cs              # Source-generated serialization (AOT-critical, all DTOs registered here)
        # ~26 DTOs covering library items, series, authors, search,
        # backups, uploads, covers, metadata providers, tasks, and
        # author match / image request-response shapes
      Configuration/
        AppConfig.cs
        ConfigManager.cs
      Output/
  ...
  tests/
    AbsCli.Tests/
      Api/            # FilterEncoder, TokenHelper, FilenameSanitizer, UnicodeNfc
      Commands/       # HelpOutput, response-shape drift, command-specific
      Configuration/  # ConfigManager
  tools/
    GenerateResponseExamples/      # Build-time tool that walks JsonContext and writes ResponseExamples.g.cs
    GenerateNfcTables/             # Manual tool that writes UnicodeNfc.g.cs from ICU
```
