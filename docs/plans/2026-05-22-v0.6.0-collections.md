...
Add a new section to `docker/smoke-test.sh` after an existing block (a good seam is right before `=== Backup Commands ===` or after `=== Author Image Commands ===`). The smoke uses `pass` / `fail` / `assert_json_key` / `assert_json_expr` helpers and the `READONLY_TOKEN` swap pattern for permission denial — match those conventions exactly.
...
- The `READONLY_TOKEN` env var is set earlier in the script (search for `uploadpass` near line 558). It will already be in scope when this block runs.
...
- [x] `collections create / update / reorder / delete / add / remove / batch-*` exercised end-to-end including duplicate-add 400 and readonlyuser permission denial
